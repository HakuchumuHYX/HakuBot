from utils.paths import project_root, shared_cache
...
if not hasattr(emoji.unicode_codes, "get_emoji_unicode_dict"):
    ...
    if isinstance(emoji.unicode_codes, type):
        emoji.unicode_codes.get_emoji_unicode_dict = staticmethod(
            _get_emoji_unicode_dict
        )
    else:
...
from utils.rendering.draw.img_utils import (
...
def deterministic_hash(obj: Any, raise_error: bool = False) -> str:
    ...
    def _serialize(o: Any):
        ...
        is_container = (
            isinstance(o, (dict, list, tuple, set, frozenset))
            or hasattr(o, "__dict__")
            or hasattr(o, "__slots__")
        )
        if is_container:
            seen.add(oid)
...
PAINTER_CACHE_DIR = str(shared_cache("painter"))
...
# 字体目录（通用）
FONT_DIR = str(project_root() / "data/shared/fonts")
...
class Gradient:
    ...
    def _lerp_color(self, t: np.ndarray, mode) -> np.ndarray:
        if mode in "RGB_OR_RGBA":
            colors = (1 - t[:, :, np.newaxis]) * np.array(self.c1) + t[
                :, :, np.newaxis
            ] * np.array(self.c2)
    ...
    def get_img(
        self, size: Size, mask: Image.Image = None, mode="RGB_OR_RGBA"
    ) -> Image.Image:
...
class LinearGradient(Gradient):
    def __init__(
        self, c1: Color, c2: Color, p1: Position, p2: Position, method: str = "seperate"
    ):
        self.c1 = c1
        self.c2 = c2
        self.p1 = p1
    ...
    def _get_colors(self, size: Size, mode: str) -> np.ndarray:
        ...
        if self.method == "combine":
            ...
            dot_product = np.sum(
                vector_p1_to_pixel * gradient_vector, axis=-1
            )  # (H, W)
...
class Painter:
    ...
    def _text(
        ...
        if not has_emoji(text):
            ...
            pos = (
                pos[0] - text_offset[0] + self.offset[0],
                pos[1] - text_offset[1] + self.offset[1],
            )
            ...
            with Pilmoji(self.img, source=GoogleEmojiSource) as pilmoji:
                ...
                pilmoji.text(
                    ...
                    fill=fill,
                    align=align,
                    emoji_position_offset=offset,
                    emoji_scale_factor=scale,
                    anchor="ls",
                )
        return self
    ...
    def _get_aa_roundrect(
        ...
        if stroke and stroke_width > 0:
            draw.rectangle(
                getbox(radius, 0, width - radius, stroke_width), fill=stroke
            )  # 上
            draw.rectangle(
                getbox(radius, height - stroke_width, width - radius, height),
                fill=stroke,
            )  # 下
            draw.rectangle(
                getbox(0, radius, stroke_width, height - radius), fill=stroke
            )  # 左
            draw.rectangle(
                getbox(width - stroke_width, radius, width, height - radius),
                fill=stroke,
            )  # 右
        ...
        corner_aa = None
        if any(corners):
            corner_canvas = Image.new(
                "RGBA", (aa_radius * 2, aa_radius * 2), (0, 0, 0, 0)
            )
            ...
            corner_aa = corner_canvas.resize(
                (radius + 1, radius + 1), Image.Resampling.BICUBIC
            )
        ...
        if not all(corners):
            ...
            if stroke and stroke_width > 0:
                sharp_draw.rectangle(
                    (0, 0, radius + 1, stroke_width), fill=stroke
                )  # 上
                sharp_draw.rectangle(
                    (0, 0, stroke_width, radius + 1), fill=stroke
                )  # 左
    ...
    @staticmethod
    def _execute(
        operations: List[PainterOperation],
        img: Image.Image,
        size: Tuple[int, int],
    ...
    async def get(self, cache_key: str = None) -> Image.Image:
        if cache_key is not None:
            op_hash = await asyncio.to_thread(
                deterministic_hash, {"key": cache_key, "op": self.operations}
            )
        ...
        with ThreadPoolExecutor() as pool:
            self.img = await loop.run_in_executor(
                pool, Painter._execute, self.operations, self.img, self.size, image_dict
            )
        ...
        if cache_key is not None:
            try:
                cache_path = os.path.join(
                    PAINTER_CACHE_DIR, f"{cache_key}__{op_hash}.png"
                )
    ...
    def add_operation(
        self, func: Union[str, callable], exclude_on_hash: bool, args: List[Any]
    ):
    ...
    def text(
        ...
        return self.add_operation(
            "_impl_text", exclude_on_hash, (text, pos, font, fill, align)
        )
    ...
    def rect(
        ...
        return self.add_operation(
            "_impl_rect", exclude_on_hash, (pos, size, fill, stroke, stroke_width)
        )
    ...
    def roundrect(
        ...
        return self.add_operation(
            "_impl_roundrect",
            exclude_on_hash,
            (pos, size, fill, radius, stroke, stroke_width, corners),
        )
        ...
        if (len(fill) == 3 or fill[3] == 255) and not gradient and not adaptive:
            ...
            if gradient:
                ...
                if adaptive.pixelwise:
                    ...
                    gray = Image.new(
                        "RGB", bg_img.size, tuple(avg_color[:3].astype(int))
                    ).convert("L")
            ...
            self.img.alpha_composite(
                overlay, (pos[0] + self.offset[0], pos[1] + self.offset[1])
            )
    ...
    def _impl_paste(
        ...
        if use_shadow:
            ...
            shadow_mask.paste(
                Image.new("L", sub_img.size, int(255 * shadow_alpha)), (sw, sw), sub_img
            )
            blurred_shadow_mask = shadow_mask.filter(
                ImageFilter.GaussianBlur(radius=sw // 2)
            )
            ...
            self.img.alpha_composite(
                shadow, (pos[0] + self.offset[0] - sw, pos[1] + self.offset[1] - sw)
            )

        if sub_img.mode == "RGBA":
            self.img.paste(
                sub_img, (pos[0] + self.offset[0], pos[1] + self.offset[1]), sub_img
            )
        else:
    ...
    def _impl_paste_with_alphablend(
        ...
        if use_shadow:
            ...
            shadow_mask = Image.new("L", (lw, lh), 0)
            shadow_mask.paste(
                Image.new("L", overlay.size, int(255 * shadow_alpha)), (sw, sw), overlay
            )
            blurred_shadow_mask = shadow_mask.filter(
                ImageFilter.GaussianBlur(radius=sw // 2)
            )
    ...
    def _impl_rect(
        ...
        if fill[3] == 255 and not gradient:
            ...
            draw.rectangle(
                (0, 0, size[0], size[1]), fill=fill, outline=stroke, width=stroke_width
            )
            if gradient:
                gradient_img = gradient.get_img(overlay_size, overlay)
                overlay = gradient_img
    ...
    def _impl_roundrect(
        ...
        overlay = self._get_aa_roundrect(
            size, fill, radius, stroke, stroke_width, corners
        )
