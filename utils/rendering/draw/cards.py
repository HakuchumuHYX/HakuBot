...
from utils.rendering.themes import LIGHT, DARK
...
class LeaderboardCardColors:
    # 画布背景（不要纯白，避免刺眼）
    canvas_bg: tuple[int, int, int, int] = (*LIGHT.canvas_bg, 255)  # 浅蓝
    # 卡片背景
    card_bg: tuple[int, int, int, int] = (*LIGHT.card_bg, 255)  # 更浅的蓝白
    # 卡片边框
    card_border: tuple[int, int, int, int] = (*LIGHT.card_border, 255)

    # 行背景
    row_bg: tuple[int, int, int, int] = (*LIGHT.row_bg, 255)

    # 文本颜色
    text_main: tuple[int, int, int, int] = (*LIGHT.text_main, 255)
    text_sub: tuple[int, int, int, int] = (*LIGHT.text_sub, 255)
    text_muted: tuple[int, int, int, int] = (*LIGHT.text_muted, 255)
...
DARK_COLORS = LeaderboardCardColors(
    canvas_bg=(*DARK.canvas_bg, 255),  # 深灰蓝底
    card_bg=(*DARK.card_bg, 255),  # 深色卡片
    card_border=(*DARK.card_border, 255),  # 低对比边框
    row_bg=(*DARK.row_bg, 255),  # 行背景
    text_main=(*DARK.text_main, 255),  # 浅白文字
    text_sub=(*DARK.text_sub, 255),  # 次要文字
    text_muted=(*DARK.text_muted, 255),  # 水印等
...
def _badge_bg_by_rank(
    colors: LeaderboardCardColors, rank: int
...
async def render_leaderboard_card(
    *,
    ...
    title_style = TextStyle(
        font="SourceHanSansCN-Heavy", size=34, color=colors.text_main
    )
    subtitle_style = TextStyle(
        font="SourceHanSansCN-Regular", size=18, color=colors.text_sub
    )
    total_label_style = TextStyle(
        font="SourceHanSansCN-Regular", size=16, color=colors.text_sub
    )
    total_value_style = TextStyle(
        font="SourceHanSansCN-Heavy", size=54, color=colors.text_main
    )

    row_name_style = TextStyle(
        font="SourceHanSansCN-Regular", size=20, color=colors.text_main
    )
    row_count_style = TextStyle(
        font="SourceHanSansCN-Bold", size=20, color=colors.text_main
    )
    ...
    footer_style = TextStyle(
        font="SourceHanSansCN-Regular", size=18, color=colors.text_main
    )
    ...
    # 标题区
    items.append(
        TextBox(title, style=title_style, wrap=True, use_real_line_count=True)
        .set_w(content_w)
        .set_padding(0)
    )
    if subtitle:
    ...
    if total_label:
        items.append(
            TextBox(total_label, style=total_label_style)
            .set_w(content_w)
            .set_padding(0)
        )
    items.append(
        TextBox(str(total), style=total_value_style).set_w(content_w).set_padding(0)
    )
    ...
    if rows_list:
        ...
        for idx, (name, count) in enumerate(rows_list, start=1):
            ...
            count_box = (
                TextBox(
                    f"{count} 条", style=row_count_style, wrap=False, overflow="shrink"
                )
                .set_w(w_count)
                .set_content_align("r")
                .set_padding(0)
            ...
            row = (
                HSplit(
                    items=[badge, name_box, count_box],
                    sep=inner_sep,
        ...
        items.append(
            VSplit(
                items=row_widgets, sep=row_sep, item_size_mode="fixed", item_align="c"
            )
            .set_w(content_w)
            .set_padding(0)
            .set_margin(0)
        ...
        items.append(
            TextBox("暂无排行数据", style=subtitle_style)
            .set_w(content_w)
            .set_padding(0)
        )
    ...
    if footer:
        ...
        items.append(
            TextBox(
                str(footer), style=footer_style, wrap=True, use_real_line_count=True
            )
            .set_w(content_w)
            .set_padding(0)
        )
