...
class Widget:
    ...
    def _get_self_size(self):
        if not all([self._calc_w, self._calc_h]):
            ...
            content_w_limit = (
                self.w - self.hpadding * 2 if self.w is not None else content_w
            )
            content_h_limit = (
                self.h - self.vpadding * 2 if self.h is not None else content_h
            )
            if content_w > content_w_limit or content_h > content_h_limit:
                if not self.allow_draw_outside:
                    # 为了避免报错，这里改为只警告并使用限制大小，或者直接使用限制大小
...
class HSplit(Widget):
    ...
    def _get_item_sizes(self):
        ratios = (
            self.ratios
            if self.ratios
            else [item._get_self_size()[0] for item in self.items]
        )
        if self.item_size_mode == "expand":
            assert self.w is not None, "Expand mode requires width"
            ratio_sum = sum(ratios)
            unit_w = (
                self.w - self.sep * (len(ratios) - 1) - self.hpadding * 2
            ) / ratio_sum
        else:
            unit_w = 0
    ...
    def _get_content_size(self):
        ...
        return (
            sum(s[0] for s in sizes) + self.sep * (len(sizes) - 1),
...
class VSplit(Widget):
    ...
    def _get_item_sizes(self):
        ratios = (
            self.ratios
            if self.ratios
            else [item._get_self_size()[1] for item in self.items]
        )
        if self.item_size_mode == "expand":
            assert self.h is not None, "Expand mode requires height"
            ratio_sum = sum(ratios)
            unit_h = (
                self.h - self.sep * (len(ratios) - 1) - self.vpadding * 2
            ) / ratio_sum
        else:
            unit_h = 0
            for r, item in zip(ratios, self.items):
    ...
    def _get_content_size(self):
        ...
        return (
            ...
            sum(s[1] for s in sizes) + self.sep * (len(sizes) - 1),
        )
            ...
            if self.w:
                ...
                if self.wrap:
                    ...
                    while True:
                        ...
                        line_suffix = (
                            suffix if len(clipped_lines) == self.line_count - 1 else ""
                        )
                        if line_suffix:
                            # 需要添加后缀，需要考虑后缀重新计算裁剪位置
                            clip_idx = self._get_clip_text_to_width_idx(
                                font, line, w, line_suffix
                            )
                            if clip_idx is None:
                                clipped_lines.append(line)
                                break
        ...
        for i, line in enumerate(lines):
            ...
            if self.style.use_shadow:
                ...
                if isinstance(so, int):
                    so = (so, so)
                p.text(
                    line, so, font=self._get_font_desc(), fill=self.style.shadow_color
                )
    ...
    def _get_content_size(self):
        ...
        if self.image_size_mode == "original":
            ...
            assert self.w is not None or self.h is not None, (
                "Fit mode requires width or height"
            )
            ...
            assert self.w is not None or self.h is not None, (
                "Fill mode requires width or height"
            )
            if self.w and self.h:
                return (
                    int(self.w - self.hpadding * 2),
                    int(self.h - self.vpadding * 2),
                )
            else:
                tw = self.w - self.hpadding * 2 if self.w else 1000000
                th = self.h - self.vpadding * 2 if self.h else 1000000
...
class Canvas(Frame):
    ...
    async def get_img(self, scale: float = None, cache_key: str = None):
        ...
        if PLOT_LOG_DRAW_TIME:
            logger.info(
                f"Canvas layouted in {(datetime.now() - t).total_seconds():.3f}s, size={size}"
            )
