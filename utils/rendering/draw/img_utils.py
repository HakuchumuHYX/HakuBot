...
class TransparentAnimatedGifConverter(object):
    ...
    def process(self) -> Image:
        ...
        self._img_p = rgb_img.quantize(
            palette=pal_img, method=_QUANTIZE_METHOD, dither=_DITHER
        )
...
def _create_animated_gif(
    ...
    for frame in images:
        ...
        converter = TransparentAnimatedGifConverter(
            img_rgba=thumbnail_rgba, alpha_threshold=alpha_threshold
        )
...
def save_transparent_static_gif(
    img: Image, save_path: str, alpha_threshold: float = 0.5
):
    """
    保存静态透明GIF图像
    """
    return save_transparent_gif(
        img, duration=50, save_path=save_path, alpha_threshold=alpha_threshold
    )
