"""HLTV 多赛事调度：轮询状态、推送与 APScheduler 任务装配。"""
...
from core.lifecycle import runtime, on_plugin_startup
...
from plugins.hltv_sub.scheduler_internal.state import get_event_state
    ...
    async def _probe_and_drain_pending_results_before_unsubscribe(
        ...
        *,
        ...
        for idx in range(1, rounds + 1):
            ...
            pending = [
                r for r in results if not data_manager.is_result_notified(r.id)
            ]
            logger.info(
    ...
    async def daily_maintenance(self) -> dict:
        ...
        for event_id in checked_events:
            ...
            if state == "ENDED":
                ...
                if can_unsubscribe and data_manager.unsubscribe_event(
                    event_id
                ):
...
{f"📋 {bo_text}" if bo_text else ""}""".strip()
    ...
    def _ensure_event_job(self, event_id: str) -> None:
        ...
        try:
            ...
            async def _scheduled_check(eid: str):
                ...
                await self.run_check_for_event(eid)
    ...
    def _ensure_daily_maintenance_job(self) -> None:
        try:
...
def setup_scheduler() -> None:
    ...
    async def start_scheduler():
        ...
        runtime.spawn(hltv_scheduler._delayed_init(), name="hltv_sub")
