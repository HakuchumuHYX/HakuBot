...
from plugins.image_processor.utils import IMAGE_PROCESSOR_MAX_VIDEO_BYTES, save_gif
...
async def download_video(
    url: str, file_name: str = None, expected_size: int = 0, access_token: str = None
) -> str:
    ...
    if "ftn.qq.com" in url:
        ...
        # 确保有fname参数
        if (
            "fname" not in query_params
            or not query_params["fname"]
            or query_params["fname"][0] == ""
        ):
            if file_name:
                # 使用提供的文件名
    ...
    if expected_size > IMAGE_PROCESSOR_MAX_VIDEO_BYTES:
        raise Exception(
            f"视频文件过大：{expected_size} bytes > {IMAGE_PROCESSOR_MAX_VIDEO_BYTES} bytes"
        )
        ...
        try:
            ...
            async with session.head(url) as head_response:
                if head_response.status not in [200, 206]:
                    ...
                    logger.info(
                        f"文件信息 - 大小: {content_length}, 类型: {content_type}"
                    )
            ...
            async with session.get(url) as response:
                if response.status in [200, 206]:
                    ...
                    if total_size > IMAGE_PROCESSOR_MAX_VIDEO_BYTES:
                        raise Exception(
                            f"视频文件过大：{total_size} bytes > {IMAGE_PROCESSOR_MAX_VIDEO_BYTES} bytes"
                        )
                    downloaded_size = 0
                            ...
                            if chunk:
                                ...
                                if downloaded_size > IMAGE_PROCESSOR_MAX_VIDEO_BYTES:
                                    try:
                                        ...
                                        pass
                                    raise Exception(
                                        f"视频文件过大：已下载 {downloaded_size} bytes > {IMAGE_PROCESSOR_MAX_VIDEO_BYTES} bytes"
                                    )
                                ...
                                if total_size > 0:
                                    ...
                                    if int(percent) % 10 == 0:  # 每10%打印一次进度
                                        logger.info(
                                            f"下载进度: {percent:.1f}% ({downloaded_size}/{total_size} bytes)"
                                        )
                                else:
                    ...
                    # 如果没有预期大小，或者大小匹配，再使用 content-length 验证
                        ...
                        logger.warning(
                            f"警告: 下载的文件可能不完整 (期望: {total_size}, 实际: {final_size})"
                        )
        ...
        try:
            ...
            if result.returncode == 0:
                ...
                if len(parts) >= 5:
                    ...
                    fps_str = (
                        parts[2] if len(parts) > 2 and parts[2] != "N/A" else "25/1"
                    )
                    ...
                    try:
                        frame_count = (
                            int(frame_count_str) if frame_count_str != "N/A" else 0
                        )
                    except:
                        frame_count = 0
...
async def convert_video_to_gif_ffmpeg(
    video_path: str, output_path: str, fps: int, width: int, height: int
):
    ...
    try:
        ...
        palette_cmd = [
            ...
            video_path,
            "-vf",
            f"fps={fps},scale={width}:{height}:flags=lanczos,palettegen=stats_mode=diff",
            "-y",
            palette_path,
        ]
...
async def convert_video_to_gif_opencv(
    video_path: str, output_path: str, fps: int, width: int, height: int
):
...
async def optimize_gif_parameters(video_info: dict) -> tuple:
    ...
    logger.info(
        f"原始视频参数: 时长{duration:.1f}s, FPS{original_fps:.1f}, 分辨率{original_width}x{original_height}"
    )
...
def check_ffmpeg_available() -> bool:
    ...
    try:
        ...
        if not _ffmpeg_available:
            logger.warning(
                "警告: ffmpeg未安装或不可用，将使用OpenCV进行视频转换，质量可能较低"
            )
            logger.warning(
                "建议安装ffmpeg以获得更好的GIF质量: sudo apt-get install ffmpeg"
            )
        else:
...
async def convert_video_to_gif(
    ...
    try:
        ...
        video_path = await download_video(
            video_url, file_name, expected_size, access_token
        )
        ...
        if duration > 60:
            raise Exception(
                f"视频时长超过限制（{duration:.1f}秒 > 60秒），请选择较短的视频"
            )
        ...
        if ffmpeg_available:
            # 优先使用ffmpeg（高质量）
            success = await convert_video_to_gif_ffmpeg(
                video_path, str(output_path), fps, width, height
            )

            if not success:
                logger.warning("ffmpeg转换失败，尝试OpenCV备选方案")
                success = await convert_video_to_gif_opencv(
                    video_path, str(output_path), fps, width, height
                )
        else:
            # 直接使用OpenCV
