...
from .gif_speed import change_gif_speed, change_gif_speed_alternative  # 新增导入
from .image_symmetry import process_image_symmetry  # 新增导入
...
gif_speed_handler = on_command("imgx", rule=to_me(), priority=5, block=True)  # 新增
image_symmetry_handler = on_command("img对称", rule=to_me(), priority=5, block=True)  # 新增
image_symmetry_left_handler = on_command("img左对称", rule=to_me(), priority=5, block=True)  # 新增
image_symmetry_right_handler = on_command("img右对称", rule=to_me(), priority=5, block=True)  # 新增
image_symmetry_center_handler = on_command("img中心对称", rule=to_me(), priority=5, block=True)  # 新增
image_symmetry_top_handler = on_command("img上对称", rule=to_me(), priority=5, block=True)  # 新增
image_symmetry_bottom_handler = on_command("img下对称", rule=to_me(), priority=5, block=True)  # 新增
...
async def handle_gif_speed(event: Event, cmd_arg: Message = CommandArg()):
    ...
    if not args:
        await gif_speed_handler.finish("请指定倍速倍数，例如：imgnx 2")
...
async def handle_image_symmetry_common(event: Event, symmetry_type: str):
    ...
    try:
        ...
        if result_path and os.path.exists(result_path):
            file_size = os.path.getsize(result_path)
            if file_size > 100:
                await image_symmetry_handler.finish(MessageSegment.image(f"file:///{result_path}"))
            else:
                await image_symmetry_handler.finish(f"图片{symmetry_name}处理失败，生成的文件异常")
        else:
            await image_symmetry_handler.finish(f"图片{symmetry_name}处理失败")
    except FinishedException:
        raise
    except Exception as e:
        await image_symmetry_handler.finish(f"处理图片时出错: {str(e)}")
