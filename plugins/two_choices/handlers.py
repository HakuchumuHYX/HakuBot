...
# 配置文件路径
CONFIG_FILE = PluginPaths("two_choices").config / "config.json"
    ...
    try:
        if CONFIG_FILE.exists():
            with open(CONFIG_FILE, "r", encoding="utf-8") as f:
                ...
                plugin_config["error_responses"] = config_data.get(
                    "error_responses", []
                )
            logger.info(
            ...
            atomic_write_json(CONFIG_FILE, default_config, ensure_ascii=False, indent=2)
...
def _extract_by_separator(body: str) -> Optional[Tuple[str, str]]:
    ...
    # 优先匹配更长的分隔词
    sep_pattern = "|".join(
        re.escape(s) for s in sorted(separators, key=len, reverse=True)
    )
...
# 插件启动时加载配置
@on_plugin_startup(get_driver(), "two_choices")
...
reload_config = on_command(
    "重载二择配置", permission=SUPERUSER, rule=to_me(), priority=5, block=True
)
...
# 查看当前配置的命令
view_config = on_command(
    "查看二择配置", permission=SUPERUSER, rule=to_me(), priority=5, block=True
)
