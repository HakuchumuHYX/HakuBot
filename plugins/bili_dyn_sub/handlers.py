...
from plugins.bili_dyn_sub.backoff import backoff_manager
from plugins.bili_dyn_sub.config import Config, plugin_config
from plugins.bili_dyn_sub.credential import credential_manager
from plugins.bili_dyn_sub.parser import CATEGORY_NAMES, parse_feed
from plugins.bili_dyn_sub.render import build_messages, build_text
from plugins.bili_dyn_sub.store import store

# 导入 scheduler 以注册定时任务（import 即注册），并复用其发送分发逻辑
from plugins.bili_dyn_sub import scheduler as _scheduler
from plugins.bili_dyn_sub.services.delivery import SendTarget
from plugins.bili_dyn_sub.services.delivery import dispatch_segments
...
async def membership_warning(bot: Bot, group_id: int) -> str:
    ...
    try:
        groups = await asyncio.wait_for(
            bot.get_group_list(), timeout=_GROUP_LIST_TIMEOUT
        )
        joined = {
            int(g["group_id"])
            for g in groups
...
def render_group_list(group_id: int) -> str:
    ...
    lines = [
        f"📋 群 {group_id} 的 B 站动态订阅（{len(items)} 个）：",
        _format_global_status(),
    ]
...
def render_global_list() -> str:
    ...
    group_count = len(
        {group_id for item in items for group_id in item.get("groups") or []}
    )
    lines = [
...
@bili_sub.handle()
async def handle_subscribe_group(
...
@bili_sub.handle()
async def handle_subscribe_private(
...
@bili_unsub.handle()
async def handle_unsubscribe_group(
    bot: Bot, event: GroupMessageEvent, args: Message = CommandArg()
):
    ...
    if group_raw:
        target = parse_group_id(group_raw)
        if target is None:
            await bili_unsub.finish(
                _group_id_usage("b站退订", group_raw, private=False)
            )
        group_id = target
...
@bili_unsub.handle()
async def handle_unsubscribe_private(
    bot: Bot, event: PrivateMessageEvent, args: Message = CommandArg()
):
    """私聊：b站退订 <UID> <群号> —— 群号必填"""
    uid_raw, group_raw = split_command_args(args.extract_plain_text())
