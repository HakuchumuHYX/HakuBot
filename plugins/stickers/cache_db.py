...
from plugins.stickers.send import sticker_dir
from plugins.stickers.config import CACHE_VERSION, CACHE_TTL
...
def _get_connection() -> sqlite3.Connection:
    ...
    if not hasattr(_local, "connection") or _local.connection is None:
        _local.connection = sqlite3.connect(
            str(DB_FILE), timeout=30.0, check_same_thread=False
        )
...
def init_database():
    ...
    with _init_lock:
        ...
        with get_db() as conn:
            ...
            # 检查版本
            cursor = conn.execute("SELECT value FROM cache_meta WHERE key = 'version'")
            row = cursor.fetchone()
...
def invalidate_cache(file_path: Path):
    ...
    with get_db() as conn:
        conn.execute("DELETE FROM hash_cache WHERE file_path = ?", (file_path_str,))
...
def cleanup_expired():
    ...
    with get_db() as conn:
        cursor = conn.execute(
            "DELETE FROM hash_cache WHERE created_at < ?", (expire_before,)
        )
...
def cleanup_orphaned():
    ...
    with get_db() as conn:
        ...
        if ids_to_delete:
            ...
            conn.execute(
                f"DELETE FROM hash_cache WHERE id IN ({placeholders})", ids_to_delete
            )
