...
from plugins.stickers.config import IMAGE_EXTENSIONS

# 插件数据目录
sticker_dir: Path = PluginPaths("stickers").data
...
def scan_sticker_folders_fallback() -> None:
    ...
    logger.warning(
        f"回退模式扫描完成，找到 {len(sticker_folders)} 个贴图文件夹: {list(sticker_folders.keys())}"
    )


def create_default_list_json() -> None:
    """创建默认的 list.json 文件"""
    default_data = {"folders": [{"name": "example", "aliases": ["demo", "示例"]}]}

    try:
        atomic_write_json(list_json_path, default_data, ensure_ascii=False, indent=2)
...
def get_folder_display_info() -> List[Dict]:
    ...
    for folder_config in folder_configs:
        ...
        result.append(
            {"name": folder_name, "aliases": aliases, "image_count": image_count}
        )
