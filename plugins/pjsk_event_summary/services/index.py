...
def rebuild_event_index(*, reason: str = "manual", commit: str = "") -> dict[str, Any]:
    ...
    try:
        ...
        atomic_write_json(EVENT_INDEX_FILE, events)
...
async def ensure_event_detail_path(
    event_id: int, *, auto_rebuild: bool = True
) -> Path | None:
...
def main(argv: Sequence[str] | None = None) -> int:
    ...
    parser.add_argument(
        "--repo-dir", default="", help="override local MoeSekai-Hub repository path"
    )
    parser.add_argument(
        "--index-dir", default="", help="override local MoeSekai-Hub index directory"
    )
    parser.add_argument(
        "--reason", default="manual", help="reason written into state.json"
    )
    parser.add_argument(
        "--commit", default="", help="source repository commit written into state.json"
    )
