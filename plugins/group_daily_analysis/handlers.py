...
from plugins.group_daily_analysis.analysis.main import MessageAnalyzer
from plugins.group_daily_analysis.render.renderer import ReportRenderer
from plugins.group_daily_analysis.data_source import MessageFetcher
from plugins.group_daily_analysis.database import db
...
message_sent_recorder = on_type(
    OneBotEvent,
    rule=lambda event: (
        getattr(event, "post_type", None) == "message_sent"
...
async def run_analysis(bot: Bot, group_id: int, retries: int = 3, debug: bool = False):
    ...
    if not debug and len(messages) < plugin_config.min_messages_threshold:
        logger.warning(
            f"群 {group_id} 消息数量不足 ({len(messages)} < {plugin_config.min_messages_threshold})，跳过分析"
        )
        return None
...
async def handle_analysis(bot: Bot, event: GroupMessageEvent):
    ...
    try:
        ...
        if image_bytes:
            ...
            await analysis_cmd.finish(
                f"消息数量不足 {plugin_config.min_messages_threshold} 条，无法生成总结。"
            )
...
TEMPLATES_DIR = Path(__file__).parent / "render" / "templates"
...
async def handle_set_template(bot: Bot, event: GroupMessageEvent):
    ...
    from plugins.group_daily_analysis.config import CONFIG_PATH

    update_fields(CONFIG_PATH, {"report_template": args})
...
async def auto_run_daily_analysis():
    ...
    for group_id_str in target_groups:
        try:
            ...
            image_bytes = await run_analysis(
                bot, group_id, retries=3
            )  # 自动任务重试3次
            if image_bytes:
                resp = await bot.send_group_msg(
                    group_id=group_id, message=image_segment(image_bytes)
