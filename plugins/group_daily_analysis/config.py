...
class PluginConfig(StrictBaseModel):
    ...
    max_messages: int = (
        0  # 设为 0 表示不限制最大消息数，依靠 LLM 长窗口和自适应算法处理
    )
    analysis_days: int = 1
    ...
    max_input_length: int = (
        30000  # 字符数阈值，超过则分块 (建议调大以发挥现代长窗口模型优势)
    )
...
CONFIG_PATH = PluginPaths("group_daily_analysis").config / "config.json"


def load_config() -> PluginConfig:
    config = read_config_model("group_daily_analysis", PluginConfig)
