...
from plugins.group_daily_analysis.models import ActivityVisualization
...
class ActivityVisualizer:
    def generate_activity_visualization(self, messages: list) -> ActivityVisualization:
        ...
        peak_hours = [
            ...
            if count > avg_hourly and count > 0
        ]
        ...
        for user_id, count in sorted(
            ...
            for msg in messages:
                ...
                if str(sender.get("user_id", "")) == user_id:
                    user_name = (
                        sender.get("card") or sender.get("nickname") or "未知用户"
                    )
                    break

            user_activity_ranking.append(
                {"user_id": user_id, "user_name": user_name, "message_count": count}
            )
