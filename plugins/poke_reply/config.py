...
DATA_DIR = PluginPaths("poke_reply").data
...
def load_config():
    ...
    if TEXT_TO_IMAGE_GROUPS_FILE.exists():
        result = load_json(
            TEXT_TO_IMAGE_GROUPS_FILE,
            expected_type=list,
            missing_ok=True,
            backup_on_error=True,
            default=list(DEFAULT_TEXT_TO_IMAGE_GROUPS),
        )
        if result.success:
            TEXT_TO_IMAGE_ENABLED_GROUPS = set(result.data)
            logger.info(
                f"已加载文本转图片群组配置: {len(TEXT_TO_IMAGE_ENABLED_GROUPS)} 个群组"
            )
        else:
...
def save_text_to_image_groups():
    ...
    try:
        atomic_write_json(
            TEXT_TO_IMAGE_GROUPS_FILE,
            list(TEXT_TO_IMAGE_ENABLED_GROUPS),
            expected_type=list,
        )
        logger.debug(
            f"已保存文本转图片群组配置: {len(TEXT_TO_IMAGE_ENABLED_GROUPS)} 个群组"
        )
