...
from nonebot.adapters.onebot.v11 import PokeNotifyEvent, Message, MessageSegment, Bot
from nonebot.exception import FinishedException

from plugins.poke_reply.config import (
    is_text_to_image_enabled,
    TEXT_TO_IMAGE_LENGTH_THRESHOLD,
)
...
from plugins.poke_reply.utils.common import get_group_id
from plugins.poke_reply.services.text import convert_text_to_image
...
async def handle_poke(bot: Bot, event: PokeNotifyEvent):
    ...
    if (
        ...
        or event.sub_type != "poke"
        or event.target_id != event.self_id
    ):
        return
    ...
    if not is_superuser:
        ...
        if remaining_time > 0:
            logger.info(
                f"群 {group_id} 用户 {user_id} 戳一戳CD中，剩余 {remaining_time}秒"
            )
            return
    ...
    try:
        if not data_manager.ensure_group_data_loaded(group_id):
            await bot.send_group_msg(
                group_id=group_id, message="数据加载失败，请联系管理员喵！"
            )
            return
        ...
        if total_count == 0:
            await bot.send_group_msg(
                group_id=group_id, message="这个群还没有投稿内容喵，快来投稿吧！"
            )
            return

        choice = random.choices(
            ["text", "image"], weights=[text_count, image_count], k=1
        )[0]

        if choice == "text":
            selected_text = data_manager.get_random_text(group_id)
            if (
                is_text_to_image_enabled(group_id)
                and len(selected_text) > TEXT_TO_IMAGE_LENGTH_THRESHOLD
            ):
                success, image_data = await convert_text_to_image(
                    selected_text, group_id
                )
                if success and image_data:
                    image_hash = hashlib.md5(image_data).hexdigest()
                    result = await bot.send_group_msg(
                        group_id=group_id, message=Message(image_segment(image_data))
                    )
                    ...
                    result = await bot.send_group_msg(
                        group_id=group_id, message=selected_text
                    )
                    cache_message_direct(
                        group_id=group_id,
                        message_id=result["message_id"],
            ...
            if image_path and os.path.exists(image_path):
                result = await bot.send_group_msg(
                    group_id=group_id, message=Message(image_segment(image_path))
                )
