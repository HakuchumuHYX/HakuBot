...
from nonebot.adapters.onebot.v11 import Message, GroupMessageEvent, MessageEvent, Bot
from nonebot.rule import Rule, to_me
from nonebot.params import CommandArg
from nonebot.exception import FinishedException

from plugins.poke_reply.config import CONTRIBUTE_COMMAND_PRIORITY
from plugins.poke_reply.models.cache import message_cache, text_image_cache
from plugins.poke_reply.utils.common import (
    get_group_id,
    extract_image_data,
    ensure_at_me,
    create_forward_message,
)
from utils.network.images import fetch_image
from plugins.poke_reply.services.contribute import (
    handle_text_contribution,
    handle_image_contribution,
)
...
contribute = on_command(
    "投稿",
    rule=ensure_at_me() & to_me(),
    priority=CONTRIBUTE_COMMAND_PRIORITY,
    block=True,
)
mention_handler = on_message(rule=to_me(), priority=15, block=False)
convert_to_text = on_message(
    rule=to_me() & Rule(_convert_to_text_rule), priority=10, block=True
)


# --- 辅助函数：缓存 ---
def cache_message_direct(
    group_id: int,
    message_id: int,
    content: str,
...
async def handle_contribute(
    ...
    if replied_message:
        ...
        if has_images:
            await process_image_contribution(
                bot, group_id, images, PLUGIN_ID_CONTRIB, user_id, is_superuser
            )
        else:
            text_to_add = target_message.extract_plain_text().strip()
            if not text_to_add:
                await contribute.finish("回复的消息没有文本内容喵！")
            await process_text_contribution(
                bot, group_id, text_to_add, PLUGIN_ID_CONTRIB, user_id, is_superuser
            )
    else:
        has_images, images = extract_image_data(target_message)
        if has_images:
            ...
            await process_text_contribution(
                bot, group_id, args_text, PLUGIN_ID_CONTRIB, user_id, is_superuser
            )


async def process_text_contribution(
    bot: Bot, group_id: int, text: str, plugin_id: str, user_id: str, is_superuser: bool
):
...
async def process_image_contribution(
    bot: Bot,
    group_id: int,
    images: list,
    plugin_id: str,
    ...
    success, message, saved_filenames = await handle_image_contribution(
        group_id, images
    )
    if success:
        if not is_superuser:
