...
from plugins.poke_reply.services import text as text_service
from utils.network.images import fetch_image
from plugins.poke_reply.config import get_group_image_dir, MAX_TEXT_LENGTH
...
async def handle_text_contribution(group_id: int, text: str) -> Tuple[bool, str]:
    ...
    if not text:
        return (
            False,
            "请提供要投稿的内容，格式：@我 投稿 你的文本，或回复一条消息 @我 投稿",
        )
    ...
    if data_manager.add_text(group_id, text):
        ...
        return (
            True,
            f"文本投稿成功！当前群共有{text_count}条文本和{image_count}张图片喵",
        )
    else:
        return False, "投稿失败，请稍后重试喵"


async def handle_image_contribution(
    group_id: int, images: list
    ...
    for img_type, img_data, segment in images:
        if img_type == "image":
            try:
                downloaded = await fetch_image(img_data)
                image_bytes, extension = downloaded.data, downloaded.extension
            except Exception:
                logger.error(f"下载图片失败: {img_data}")
                return False, "图片投稿失败，下载图片时出错喵", []

            try:
                is_duplicate, existing_name = await image_service.check_duplicate_image(
