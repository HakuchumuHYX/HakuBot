...
from plugins.poke_reply.config import (
    IMAGE_HASH_CACHE_FILE,
    CACHE_VERSION,
    IMAGE_HASH_CACHE_TTL,
    ...
    with _hash_cache_lock:
        ...
        try:
            result = load_json(
                ...
                default={"version": CACHE_VERSION, "entries": {}},
            )
...
def save_hash_cache():
    ...
    with _hash_cache_lock:
        if _hash_cache is None:
            return
        try:
            atomic_write_json(IMAGE_HASH_CACHE_FILE, _hash_cache, expected_type=dict)
        except Exception as e:
    ...
    try:
        ...
        if HAS_NUMPY:
            ...
            if len(pixels1) != len(pixels2):
                return False
            total_diff = sum(
                sum(abs(c1 - c2) for c1, c2 in zip(p1, p2))
...
def _verify_duplicate_bytes_vs_path_sync(img_path_1: Path, img_bytes_2: bytes) -> bool:
    ...
    try:
        with (
            Image.open(img_path_1) as image1,
            Image.open(io.BytesIO(img_bytes_2)) as image2,
        ):
...
def _check_duplicate_image_sync(
    group_id: int, new_image_bytes: bytes
...
async def check_duplicate_image(
    ...
    return await asyncio.to_thread(
        _check_duplicate_image_sync, group_id, new_image_bytes
    )
...
def _find_group_duplicates_sync(group_id: int) -> List[Tuple[Path, Path]]:
    ...
    for filename in image_files:
        ...
        if p_hash in perceptual_hashes:
            ...
            if f_hash == existing_f_hash and _verify_duplicate_path_vs_path_sync(
                existing_img, img_path
            ):
...
def safe_remove_group_duplicates(
    group_id: int, duplicates: List[Tuple[Path, Path]]
) -> int:
    """(SU清理用) 删除重复图片"""
    ...
    files_to_remove_names: Set[str] = {
        remove_path.name for (_, remove_path) in duplicates
    }
