...
class BaseMusicPlayer(ABC):
    ...
    @abstractmethod
    async def fetch_songs(self, keyword: str, limit: int, extra: str | None = None) -> list[Song]:
        """
        搜索歌曲
        :param keyword: 搜索关键字
        :param limit: 搜索数量
        :param extra: 额外参数（例如触发词/命令，用于平台识别）
        """
    ...
    async def fetch_extra(self, song: Song) -> Song:
        """
        默认获取额外信息的实现（音频、封面、歌词等）
        这里沿用原插件的 meting 聚合接口（稳定、简单）。
        """
    ...
    async def fetch_comments(self, song: Song) -> Song:
        """
        默认获取热门评论的实现（网易云 weapi）。
        依赖 enc_params / enc_sec_key（来自原插件的默认配置）。
        """
        if song.comments:
            return song
    ...
    async def _request(
        ...
        *,
        ...
        async with self.session.get(url, headers=headers, cookies=cookies, ssl=ssl) as resp:
            return await self._parse_response(resp)
    ...
    async def _parse_response(self, resp: aiohttp.ClientResponse):
        try:
            ...
            if resp.status != 200:
                logger.warning(f"[ai_assistant.music] HTTP 请求返回 {resp.status}: {resp_text[:200]}")
                return None

            if not resp_text.strip():
                logger.warning("[ai_assistant.music] HTTP 响应为空")
                return None
            ...
            logger.warning(f"[ai_assistant.music] 解析响应失败: {e}")
            return None
