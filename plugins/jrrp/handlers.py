...
jrrp = on_command(
    "jrrp", permission=GROUP, priority=10, rule=create_exact_command_rule("jrrp")
)
message: List[dict] = [
    {
        "expr": "jrrp == 100",
    ...
    {"expr": "jrrp == 99", "end": "！但不是 100……"},
    {"expr": "jrrp >= 90", "end": "！好评如潮！"},
    {"expr": "jrrp >= 60", "end": "！是不错的一天呢！"},
    {"expr": "jrrp > 50", "end": "！还行啦还行啦。"},
    {"expr": "jrrp == 50", "end": "！五五开……"},
    {"expr": "jrrp >= 40", "end": "！还……还行吧……？"},
    {"expr": "jrrp >= 11", "end": "！呜哇……"},
    {"expr": "jrrp >= 1", "end": "……（没错，是百分制）"},
    {"expr": "True", "end": "……"},
]
    ...
    num = round(
        abs(
            (
                ...
                / 3
                + get_hash(
                    "".join(["QWERTY", string, "0*8&6", str(now.tm_mday), "kjhg"])
                )
                / 3
            )
            / 527
