...
def generate(source_dir: str, output_path: str) -> bool:
        ...
        cleaned_chars = [
            {
                ...
                "characterType": c.get("characterType"),
            }
            for c in raw_chars
        ]
        cleaned_vocal = {
    ...
    for music in musics_data:
        ...
        song_obj = {
            ...
            "liveTalkBackgroundAssetbundleName": music.get(
    ...
    atomic_write_json(output_path, guess_song_list, indent=2, ensure_ascii=False)

    logger.info(
        f"[generate_guess_song] 成功生成 {output_path}，共 {len(guess_song_list)} 首歌曲。"
    )
    return True
