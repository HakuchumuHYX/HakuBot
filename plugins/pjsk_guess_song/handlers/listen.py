...
from utils.onebot.media import image_segment, audio_segment
...
async def _handle_listen_command(
    ...
    if isinstance(event, GroupMessageEvent):
        if not is_feature_enabled(
            "pjsk_guess_song", "listen", str(event.group_id), user_id
        ):
    ...
    async with lock:
        ...
        if not can_listen:
            await matcher.finish(
                f"......你今天听歌的次数已达上限（{listen_limit}次），请明天再来吧......"
            )
    ...
    try:
        config = game_service.listen_modes[mode]
        song_to_play, mp3_source = await game_service.get_listen_song_and_path(
            mode, search_term
        )

        if not song_to_play or not mp3_source:
            no_match_msg = (
                config["no_match_msg"].format(search_term=search_term)
                if search_term
                else "......出错了，找不到有效的音频文件。"
            )
        ...
        msg_chain = Message(
            f"歌曲:{song_to_play['id']}. {song_to_play['title']} {config['title_suffix']}\n"
        )
        if jacket_source:
            if isinstance(jacket_source, Path):
                msg_chain.append(image_segment(jacket_source))
            else:
                msg_chain.append(image_segment(jacket_source))
        ...
        if isinstance(mp3_source, Path):
            await matcher.send(audio_segment(mp3_source))
        else:
...
for cmd, mode in listen_commands.items():
    ...
    on_command(
        cmd, priority=10, block=True, rule=create_exact_command_rule(cmd)
    ).handle()(create_handler(mode))

listen_normal = on_command(
    "听", priority=10, block=True, rule=create_exact_command_rule("听")
)
...
async def _(
    ...
    if not content:
        await matcher.finish(
            "......请指定要听的歌曲名称或ID。例如：听 <歌名> [sekai/vs]"
        )
        return
    ...
    try:
        ...
        (
            ...
            mp3_source,
            vocal_info,
        ) = await game_service.get_normal_song_and_path(song_query, version_type)
        ...
        if not mp3_source or not vocal_info:
            await matcher.finish(
                f'......歌曲 "{song_to_play["title"]}" 没有找到符合要求的音频文件。'
            )
            return
        ...
        msg_chain = Message(
            f"歌曲:{song_to_play['id']}. {song_to_play['title']} {version_name}\n"
        )
        if jacket_source:
            if isinstance(jacket_source, Path):
                msg_chain.append(image_segment(jacket_source))
...
async def _(
    ...
    try:
        ...
        if vocal_info is None:
            await matcher.finish(
                f'......歌曲 "{song_to_play["title"]}" 没有找到符合要求的 Another Vocal 版本。'
            )
            return
        ...
        if vocal_info == "list_versions":
            ...
            if not anov_list:
                await matcher.finish(
                    f"......歌曲 '{song_to_play['title']}' 没有 Another Vocal 版本。"
                )
                return
            ...
            for v in anov_list:
                names = [
                    cache_service.character_data.get(str(c["characterId"]), {}).get(
                        "fullName", "未知"
                    )
        ...
        mp3_source_path = await game_service.audio_processor.process_anvo_audio(
            song_to_play, vocal_info
        )
        ...
        char_names = [
            cache_service.character_data.get(str(cid), {}).get("fullName", "未知")
            for cid in char_ids
        ]
