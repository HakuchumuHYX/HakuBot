...
from plugins.pjsk_guess_song.game_data import (
    game_session_locks,
...
from plugins.pjsk_guess_song.game_session import _run_game_session
...
async def _(bot: Bot, event: MessageEvent, state: T_State):
    ...
    if plugin_config.lightweight_mode and mode_key in ["1", "2"]:
        ...
        await start_guess_song_unified.finish(
            f'......轻量模式已启用，模式"{original_mode_name}"已自动切换为普通模式。'
        )
    ...
    try:
        ...
        await db_service.consume_daily_play_attempt(
            initiator_id, initiator_name, session_id, is_independent_limit
        )
        ...
        other_songs = random.sample(
            [s for s in cache_service.song_data if s["id"] != correct_song["id"]], 11
        )
        ...
        logger.info(
            f"[猜歌插件] 新游戏开始. 答案: {correct_song['title']} (选项 {game_data['correct_answer_num']})"
        )
        ...
        if options_img_path:
            ...
            intro_messages.append(image_segment(img_path))
        ...
        answer_reveal_messages = Message(
            f"正确答案是: {game_data['correct_answer_num']}. {correct_song['title']}\n"
        )
        if jacket_source:
            if isinstance(jacket_source, Path):
                answer_reveal_messages.append(image_segment(jacket_source))
            else:
                answer_reveal_messages.append(image_segment(jacket_source))

        await _run_game_session(
            bot, event, game_data, intro_messages, answer_reveal_messages
        )
...
# --- 随机猜歌 ---
start_random_guess_song = on_command(
    "随机猜歌",
    aliases={"rgs"},
    priority=10,
    block=True,
...
async def _(bot: Bot, event: MessageEvent):
    ...
    try:
        ...
        combined_kwargs, total_score, effect_names_display, mode_name_str = (
            game_service.get_random_mode_config()
        )
        if not combined_kwargs:
            if session_id in active_game_sessions:
                active_game_sessions.pop(session_id)
            await start_random_guess_song.finish(
                "......随机模式启动失败，没有可用的效果组合。请检查资源文件。"
            )
            return

        await start_random_guess_song.send(
            f"......本轮应用效果：【{effect_names_display}】(总计{total_score}分)"
        )
        ...
        if (
            session_id in active_game_sessions
            and active_game_sessions[session_id].get("type") == "game_init"
        ):
            active_game_sessions.pop(session_id)
            last_game_end_time[session_id] = time.time()
...
# --- 猜歌手 ---
start_vocalist_game = on_command(
    "猜歌手", priority=10, block=True, rule=create_exact_command_rule("猜歌手")
)
...
async def _(bot: Bot, event: MessageEvent):
    ...
    if not cache_service.another_vocal_songs:
        await start_vocalist_game.finish(
            "......抱歉，没有找到包含 another_vocal 的歌曲，无法开始游戏。"
        )
        return
    ...
    try:
        ...
        another_vocals = [
            v for v in all_vocals if v.get("musicVocalType") == "another_vocal"
        ]
        ...
        game_data["correct_answer_num"] = (
            another_vocals.index(correct_vocal_version) + 1
        )
        ...
        def get_vocalist_name(vocal_info):
            ...
            for char in char_list:
                ...
                if char_data:
                    char_names.append(
                        char_data.get("fullName", char_data.get("name", "未知"))
                    )
                else:
        ...
        jacket_source = cache_service.get_resource_path_or_url(
            f"music_jacket/{song['jacketAssetbundleName']}.png"
        )

        intro_messages = Message(intro_text)
        if jacket_source:
            if isinstance(jacket_source, Path):
                intro_messages.append(image_segment(jacket_source))
            else:
                intro_messages.append(image_segment(jacket_source))

        correct_vocalist_name = get_vocalist_name(correct_vocal_version)
        answer_reveal_messages = Message(
            f"正确答案是: {game_data['correct_answer_num']}. {correct_vocalist_name}"
        )
