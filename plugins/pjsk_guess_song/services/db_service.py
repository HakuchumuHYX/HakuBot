...
class DBService:
    ...
    async def _ensure_user_exists(
        self, cursor: aiosqlite.Cursor, user_id: str, user_name: str
    ):
        ...
        if await cursor.fetchone() is None:
            ...
            await cursor.execute(
                f"INSERT INTO user_stats ({', '.join(columns)}) VALUES ({placeholders})",
                default_values,
            )
    ...
    async def consume_daily_play_attempt(
        self, user_id: str, user_name: str, session_id: str, is_independent: bool
    ):
        ...
        async with self._get_conn() as conn:
            ...
            async with conn.cursor() as cursor:
                ...
                if is_independent:
                    await cursor.execute(
                        "SELECT group_daily_plays FROM user_stats WHERE user_id = ?",
                        (user_id,),
                    )
                    ...
                    current_count = (
                        group_stat.get("count", 0)
                        if group_stat.get("date") == today
                        else 0
                    )
                    group_plays[session_id] = {
                        "count": current_count + 1,
                        "date": today,
                    }

                    await cursor.execute(
                        "UPDATE user_stats SET group_daily_plays = ?, user_name = ? WHERE user_id = ?",
                        (json.dumps(group_plays), user_name, user_id),
                    )
                else:
                    await cursor.execute(
                        "SELECT daily_games_played, last_played_date FROM user_stats WHERE user_id = ?",
                        (user_id,),
                    )
                    row = await cursor.fetchone()
                    daily_games = (
                        row["daily_games_played"]
                        if row and row["last_played_date"] == today
                        else 0
                    )
                    await cursor.execute(
    ...
    async def can_play(
        self, user_id: str, daily_limit: int, session_id: str, is_independent: bool
    ) -> bool:
    ...
    async def get_games_played_today(
        self, user_id: str, session_id: str, is_independent: bool
    ) -> int:
    ...
    async def record_listen_song(self, user_id: str, user_name: str):
        async with self._get_conn() as conn:
            ...
            async with conn.cursor() as cursor:
                await self._ensure_user_exists(cursor, user_id, user_name)
                await cursor.execute(
                    "SELECT daily_listen_songs, last_listen_date FROM user_stats WHERE user_id = ?",
                    (user_id,),
                )
                row = await cursor.fetchone()
                ...
                today = datetime.now().strftime("%Y-%m-%d")
                daily_listen = (
                    row["daily_listen_songs"]
                    if row and row["last_listen_date"] == today
                    else 0
                )
    ...
    async def can_listen_song(self, user_id: str, daily_limit: int) -> bool:
        async with self._get_conn() as conn:
            ...
            async with conn.cursor() as cursor:
                ...
                row = await cursor.fetchone()
                if not row or row["last_listen_date"] != datetime.now().strftime(
                    "%Y-%m-%d"
                ):
                    return True
    ...
    async def reset_guess_limit(self, target_id: str) -> bool:
        ...
        async with self._get_conn() as conn:
            res = await conn.execute(
                "UPDATE user_stats SET daily_games_played = 0 WHERE user_id = ?",
                (target_id,),
            )
            await conn.commit()
    ...
    async def reset_listen_limit(self, target_id: str) -> bool:
        ...
        async with self._get_conn() as conn:
            res = await conn.execute(
                "UPDATE user_stats SET daily_listen_songs = 0 WHERE user_id = ?",
                (target_id,),
            )
            await conn.commit()
            return res.rowcount > 0
    ...
    async def add_score(
        self, user_id: str, group_id: str, score_to_add: int, user_name: str
    ):
        """
