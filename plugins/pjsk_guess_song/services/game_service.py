...
class GameService:
    def __init__(
        ...
        self.game_effects = {
            ...
            "acc": {
                "name": "伴奏",
                "score": 1,
                "kwargs": {"play_preprocessed": "accompaniment"},
            },
            "bass": {
                "name": "纯贝斯",
                "score": 3,
                "kwargs": {"play_preprocessed": "bass_only"},
            },
            "drums": {
                "name": "纯鼓组",
                "score": 4,
                "kwargs": {"play_preprocessed": "drums_only"},
            },
            "vocals": {
                "name": "纯人声",
                "score": 1,
                "kwargs": {"play_preprocessed": "vocals_only"},
            },
        }
        self.game_modes = {
            ...
            "3": {
                "name": "AI-Assisted Twin Piano ver.",
                "kwargs": {"melody_to_piano": True},
                "score": 2,
            },
            "4": {
                "name": "纯伴奏",
        ...
        self.listen_modes = {
            "piano": {
                "name": "钢琴",
                "list_attr": "available_piano_songs",
                "file_key": "piano",
            ...
            "accompaniment": {
                "name": "伴奏",
                "list_attr": "available_accompaniment_songs",
                "file_key": "accompaniment",
            ...
            "vocals": {
                "name": "人声",
                "list_attr": "available_vocals_songs",
                "file_key": "vocals_only",
            ...
            "bass": {
                "name": "贝斯",
                "list_attr": "available_bass_songs",
                "file_key": "bass_only",
            ...
            "drums": {
                "name": "鼓组",
                "list_attr": "available_drums_songs",
                "file_key": "drums_only",
    ...
    async def get_game_clip(self, **kwargs) -> Optional[Dict]:
        ...
        for song_attempt in range(MAX_SONG_RETRIES):
            if not song:
                if preprocessed_mode:
                    available_bundles = self.cache_service.preprocessed_tracks.get(
                        preprocessed_mode, set()
                    )
                    if not available_bundles:
                        logger.error(
                            f"无法开始 {preprocessed_mode} 模式: 没有找到任何预处理的音轨文件。"
                        )
                        return None
            ...
            logger.debug(
                f"歌曲尝试 {song_attempt + 1}/{MAX_SONG_RETRIES}: 选择歌曲 '{song.get('title')}'"
            )

            vocal_version = kwargs.get("force_vocal_version")
            if preprocessed_mode:
                possible_bundles = [
                    v["vocalAssetbundleName"]
                    for v in song.get("vocals", [])
                    if v["vocalAssetbundleName"]
                    in self.cache_service.preprocessed_tracks.get(
                        preprocessed_mode, set()
                    )
                ]
                ...
                valid_piano_bundles = list(
                    all_song_bundles.intersection(
                        self.cache_service.available_piano_songs_bundles
                    )
                )
                ...
                if vocal_version:
                    ...
                    audio_source = self.cache_service.get_resource_path_or_url(
                        f"songs/{bundle_name}/{bundle_name}.mp3"
                    )
                else:
                    audio_source = None
            ...
            if not audio_source:
                logger.warning(
                    f"歌曲 '{song.get('title')}' 没有有效的音频源文件，尝试下一首。"
                )
                song = None
                continue
            ...
            if self.vocals_silence_detection and preprocessed_mode == "vocals_only":
                try:
                    ...
                    for segment_attempt in range(MAX_SEGMENT_RETRIES_PER_SONG):
                        ...
                        start_range_max = int(
                            total_duration_ms - (target_duration_s * 1000)
                        )
                        random_start_s = (
                            random.randint(start_range_min, start_range_max)
                            if start_range_min < start_range_max
                            else start_range_min
                        ) / 1000.0

                        mean_dbfs = (
                            await self.audio_processor.get_segment_mean_dbfs_ffmpeg(
                                audio_source, random_start_s, target_duration_s
                            )
                        )

                        if (
                            mean_dbfs is not None
                            and mean_dbfs > self.silence_threshold_dbfs
                        ):
                            logger.debug(
                    ...
                    logger.error(
                        "ffmpeg/ffprobe 未安装。无法执行静音检测，将禁用此功能。"
                    )
                    self.vocals_silence_detection = False  # 禁用后续检测
                    break  # 允许游戏继续
                except Exception as e:
                    logger.error(
                        f"对歌曲 '{song.get('title')}' 进行静音检测时失败: {e}，更换歌曲。"
                    )
                    song = None
                    continue
            else:
        ...
        if not song or not audio_source:
            logger.error(
                f"在 {MAX_SONG_RETRIES} 次尝试后，未能找到任何有效的歌曲和音频片段来开始游戏。"
            )
            return None
        ...
        use_slow_path = (
            is_bass_boost or has_speed_change or has_reverse or has_band_pass
        )

        clip_duration = self.config.clip_duration_seconds
        clip_path_obj = self.output_dir / f"clip_{int(time.time())}.mp3"
        mode_key = (
            kwargs.get("random_mode_name")
            or kwargs.get("play_preprocessed")
            or ("melody_to_piano" if is_piano_mode else "normal")
        )

        # 路径 1: 人声模式的快速路径
        if (
            preprocessed_mode == "vocals_only"
            and not use_slow_path
            and forced_start_ms is not None
        ):
            logger.debug("人声模式无复杂效果，使用ffmpeg快速路径进行裁剪。")
            success = await self.audio_processor.clip_audio_ffmpeg_fast(
                audio_source, clip_path_obj, forced_start_ms, clip_duration
            )
            if success:
                return {
                    "song": song,
        ...
        if not use_slow_path:
            try:
                ...
                #  调用 audio_processor
                success = await self.audio_processor.clip_audio_ffmpeg_fast(
                    audio_source, clip_path_obj, start_ms, target_duration_ms / 1000.0
                )
                if not success:
                    raise RuntimeError("ffmpeg clipping failed")
        ...
        try:
            ...
            clip = await loop.run_in_executor(
                self.executor,
                self.audio_processor.process_audio_with_pydub,
                audio_data,
                "mp3",
                pydub_kwargs,
            )
            ...
            logger.error(
                f"慢速路径 (pydub) 处理音频文件 {audio_source} 时失败: {e}",
                exc_info=True,
            )
            return None
    ...
    def get_random_mode_config(self) -> Tuple[Dict, int, str, str]:
        ...
        target_distribution = self._get_random_target_distribution(
            combinations_by_score
        )
        ...
        if speed_mult and has_reverse:
            effect_names_display = [
                n for n in effect_names_display if n not in ["倒放", "2倍速", "1.5倍速"]
            ]
            effect_names_display.append(f"倒放+{speed_mult}倍速组合(+1分)")

        mode_name_str = "+".join(
            sorted(
                [name.replace(" ver.", "") for name in effect_names if name != "Off"]
            )
        )
        return (
            combined_kwargs,
            total_score,
            "、".join(effect_names_display),
            mode_name_str,
        )
    ...
    def _precompute_random_combinations(self) -> Dict[int, List[Dict]]:
        ...
        for effect in active_base_effects:
            independent_options.append(
                [effect, {"name": "Off", "score": 0, "kwargs": {}}]
            )
        ...
        for source_effect in playable_source_effects:
            for independent_choices in itertools.product(*independent_options):
                ...
                has_reverse_effect = any(
                    "reverse_audio" in choice.get("kwargs", {})
                    for choice in independent_choices
                )
                if is_piano_mode and has_reverse_effect:
                    continue

                raw_combination = [source_effect] + [
                    choice for choice in independent_choices if choice["score"] > 0
                ]
                final_effects_list = []
                final_kwargs = {}
                base_score = 0
                ...
                for effect_template in raw_combination:
                    effect = {
                        ...
                        for k, v in effect_template.items()
                    }
                    if is_multi_effect and "speed_multiplier" in effect.get(
                        "kwargs", {}
                    ):
                ...
                if (
                    "speed_multiplier" in final_kwargs
                    and "reverse_audio" in final_kwargs
                ):
                    final_score += 1
    ...
    def _get_random_target_distribution(
        self, combinations_by_score: Dict[int, list]
    ) -> Dict[int, float]:
    ...
    async def get_listen_song_and_path(
        self, mode: str, search_term: Optional[str]
    ) -> Tuple[Optional[Dict], Optional[Union[Path, str]]]:
        """获取听歌模式的歌曲和文件路径。"""
        ...
        if search_term:
            song_to_play = self.cache_service.find_song_by_query(
                search_term, pool=available_songs
            )
        else:
        ...
        if config["is_piano"]:
            all_song_bundles = {
                v["vocalAssetbundleName"] for v in song_to_play.get("vocals", [])
            }
            valid_piano_bundles = list(
                all_song_bundles.intersection(
                    self.cache_service.available_piano_songs_bundles
            ...
            if (
                bundle_name
                and bundle_name
                in self.cache_service.preprocessed_tracks[config["file_key"]]
            ):
    ...
    async def get_normal_song_and_path(
        self, song_query: Optional[str], version_type: str
    ) -> Tuple[Optional[Dict], Optional[Union[Path, str]], Optional[Dict]]:
        """
        ...
        vocal_version = next(
            (
                v
                for v in song_to_play.get("vocals", [])
                if v.get("musicVocalType") == version_type
            ),
            None,
        )
            ...
            if version_type == "sekai":
                vocal_version = next(
                    (
                        v
                        for v in song_to_play.get("vocals", [])
                        if v.get("musicVocalType") == "virtual_singer"
                    ),
                    None,
                )
    ...
    async def get_anvo_song_and_vocal(
        self, content: str
    ) -> Tuple[Optional[Dict], Optional[Union[Dict, str]]]:
        ...
        if not content:
            if not another_vocal_songs:
                return None, None
            song_to_play = random.choice(another_vocal_songs)
            anov_list = [
                v
                for v in song_to_play.get("vocals", [])
                if v.get("musicVocalType") == "another_vocal"
            ]
            if anov_list:
                vocal_info = random.choice(anov_list)
        else:
                ...
                if song_to_play:
                    for v in song_to_play.get("vocals", []):
                        if (
                            ...
                            and {c.get("characterId") for c in v.get("characters", [])}
                            == target_ids
                        ):
                            vocal_info = v
                            break
                    ...
                    if songs_by_char:
                        ...
                        solo = next(
                            (
                                ...
                                for v in song_to_play.get("vocals", [])
                                if v.get("musicVocalType") == "another_vocal"
                                and len(v.get("characters", [])) == 1
                                and v["characters"][0].get("characterId") == char_id
                            ),
                            None,
                        )
                        vocal_info = solo or next(
                            (
                                v
                                for v in song_to_play.get("vocals", [])
                                if v.get("musicVocalType") == "another_vocal"
                                and any(
                                    c.get("characterId") == char_id
