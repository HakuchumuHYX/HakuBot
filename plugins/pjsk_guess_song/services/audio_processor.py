...
class AudioProcessor:
    ...
    async def get_segment_mean_dbfs_ffmpeg(
        self, file_path: Union[Path, str], start_s: float, duration_s: float
    ) -> Optional[float]:
    ...
    def get_duration_ms_ffprobe_sync(
    ...
    def process_audio_with_pydub(
        self, audio_data: Union[str, Path, io.BytesIO], audio_format: str, options: dict
        ...
        try:
            ...
            if speed_multiplier != 1.0:
                clip = clip._spawn(
                    clip.raw_data,
                    overrides={"frame_rate": int(clip.frame_rate * speed_multiplier)},
                )
    ...
    async def clip_audio_ffmpeg_fast(
        self,
    ...
    async def get_audio_data(
        self, audio_source: Union[Path, str]
    ) -> Optional[Union[str, Path, io.BytesIO]]:
        """[新] 智能获取音频数据，如果是URL则下载到内存"""
        try:
            if isinstance(audio_source, str) and audio_source.startswith(
                ("http://", "https://")
            ):
