...
class ImageService:
    ...
    async def create_options_image(self, options: List[Dict]) -> Optional[str]:
        ...
        if not options or len(options) != 12:
            return None
        tasks = [
            self.cache_service.open_image(
                f"music_jacket/{opt['jacketAssetbundleName']}.png"
            )
            for opt in options
        ]
    ...
    def _draw_options_image_sync(
        self, options: List[Dict], jacket_images: List[Optional[Image.Image]]
    ) -> Optional[str]:
        """[同步] 选项图片绘制函数"""
        ...
        try:
            ...
            for y in range(img_h):
                r = int(
                    bg_color_start[0]
                    + (bg_color_end[0] - bg_color_start[0]) * y / img_h
                )
                g = int(
                    bg_color_start[1]
                    + (bg_color_end[1] - bg_color_start[1]) * y / img_h
                )
                b = int(
                    bg_color_start[2]
                    + (bg_color_end[2] - bg_color_start[2]) * y / img_h
                )
        ...
        try:
            ...
            title_font = load_font_from_path(font_path, 16)
            num_font = load_font_from_path(font_path, 24)
            id_font = load_font_from_path(font_path, 14)
        except IOError:
        ...
        for i, option in enumerate(options):
            ...
            try:
                # 处理封面图片
                jacket = jacket_img.convert("RGBA").resize(
                    (jacket_w, jacket_h), LANCZOS
                )
                ...
                jacket_draw.text(
                    (text_x, text_y), num_text, font=num_font, fill=(255, 255, 255)
                )
                ...
                if title_font.getbbox(title)[2] > jacket_w:
                    while (
                        title_font.getbbox(title + "...")[2] > jacket_w
                        and len(title) > 1
                    ):
                        title = title[:-1]
                    title += "..."
    ...
    async def draw_leaderboard_image(
        self, group_name: str, leaderboard_data: List[Tuple[str, int]]
    ) -> Optional[str]:
    ...
    def _draw_leaderboard_image_sync(
        self, group_name: str, leaderboard_data: List[Tuple[str, int]]
    ) -> Optional[str]:
        """[同步] 排行榜图片绘制函数。"""
        try:
            ...
            for y in range(height):
                r = int(
                    bg_color_start[0]
                    + (bg_color_end[0] - bg_color_start[0]) * y / height
                )
                g = int(
                    bg_color_start[1]
                    + (bg_color_end[1] - bg_color_start[1]) * y / height
                )
                b = int(
                    bg_color_start[2]
                    + (bg_color_end[2] - bg_color_start[2]) * y / height
                )
            ...
            try:
                font_path = str(self.resources_dir / "font.ttf")
                title_font = load_font_from_path(font_path, 48)
                header_font = load_font_from_path(font_path, 28)
                entry_font = load_font_from_path(font_path, 36)
                score_font = load_font_from_path(font_path, 36)
                id_font = load_font_from_path(font_path, 16)
            except IOError:
                logger.warning("未找到字体文件 font.ttf，将使用默认字体。")
                title_font = ImageFont.load_default()
            ...
            draw.text(
                (center_x - w / 2 + 2, current_y - h / 2 + 2),
                title_text,
                font=title_font,
                fill=shadow_color,
            )
            draw.text(
                (center_x - w / 2, current_y - h / 2),
                title_text,
                font=title_font,
                fill=font_color,
            )
            ...
            if self.config.custom_footer_text:
                ...
                w = bbox[2] - bbox[0]
                draw.text(
                    (center_x - w / 2, footer_y_2),
                    self.config.custom_footer_text,
                    font=id_font,
                    fill=header_color,
                )
    ...
    async def draw_resource_version_image(
        self, stats: Dict[str, int], external_info: str
    ) -> Optional[str]:
        """异步绘制资源版本统计图片 (美化版)"""
        loop = asyncio.get_running_loop()
        try:
            return await loop.run_in_executor(
                self.executor,
                self._draw_resource_version_image_sync,
                stats,
                external_info,
            )
        except Exception as e:
    ...
    def _draw_resource_version_image_sync(
        self, stats: Dict[str, int], external_info: str
    ) -> Optional[str]:
        ...
        try:
            ...
            if background_path.exists():
                try:
                    ...
                    custom_bg = Image.open(background_path).convert("RGBA")
                    custom_bg = ImageOps.fit(
                        custom_bg, (width, height), method=LANCZOS, centering=(0.5, 0.5)
                    )
                    white_overlay = Image.new(
                        "RGBA", (width, height), (255, 255, 255, 80)
                    )
            ...
            try:
                font_path = str(self.resources_dir / "font.ttf")
                title_font = load_font_from_path(font_path, 46)
                card_label_font = load_font_from_path(font_path, 22)
                card_value_font = load_font_from_path(font_path, 40)
                version_label_font = load_font_from_path(font_path, 20)
                version_val_font = load_font_from_path(font_path, 28)
                # 专门加载一个 size=16 的字体用于水印，与猜歌帮助保持一致
                watermark_font = load_font_from_path(font_path, 16)
            except IOError:
                title_font = ImageFont.load_default()
            ...
            shadow_offset = 2
            draw.text(
                (center_x - title_w / 2 + shadow_offset, title_y + shadow_offset),
                title_text,
                font=title_font,
                fill=(255, 255, 255, 150),
            )
            draw.text(
                (center_x - title_w / 2, title_y),
                title_text,
                font=title_font,
                fill=(30, 30, 50),
            )
            ...
            for i, (label, value, color) in enumerate(grid_data):
                ...
                bar_width = 8
                draw.rounded_rectangle(
                    [x, y, x + bar_width + 5, y + card_height], radius=15, fill=color
                )
                draw.rectangle(
                    [x + bar_width, y, x + bar_width + 5, y + card_height],
                    fill=(255, 255, 255, 210),
                )
            ...
            draw.text(
                (center_x, capsule_y1 + 25),
                dv_label,
                font=version_label_font,
                fill=(70, 130, 180),
                anchor="mm",
            )
            draw.text(
                (center_x, capsule_y1 + 60),
                dv_val,
                font=version_val_font,
                fill=(30, 30, 30),
                anchor="mm",
            )
