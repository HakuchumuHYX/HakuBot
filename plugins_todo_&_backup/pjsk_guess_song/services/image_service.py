...
class ImageService:
    ...
    def _draw_help_image_sync(self) -> Optional[str]:
        """[同步] 帮助图片绘制函数。"""
        # (注意: 此处硬编码了 game_modes，如果 game_service.py 中的 modes 变化，这里需要手动同步)
        # (一个更好的方案是将 game_modes 字典也传入此函数，但为了减少耦合，暂时保持现状)
        game_modes = {
    ...
    def _draw_help_text(self, draw_func, img, title_font, section_font, body_font, id_font, special_font, help_text,
        ...
        for line in lines[2:]:
            ...
            is_special_line = False  # (原版逻辑 "♿" in line 已移除)
            if is_special_line:
                font = special_font
    ...
    def _draw_leaderboard_image_sync(self, group_name: str, leaderboard_data: List[Tuple[str, int]]) -> Optional[str]:
        ...
        try:
            ...
            for i, (name, score) in enumerate(leaderboard_data, 1):
                ...
                # --- [BUGFIX] ---
                # 移除 try...except 和 anchor='rm'
                # 统一使用手动计算的右对齐，确保 Y 轴 (current_y) 与左侧文本对齐

                # 1. 获取文本宽度
                try:
                    # Pilmoji / Pillow >= 9.2.0
                    ...
                    try:
                        ...
                        # 最后的后备方案 (估算宽度)
                        w = len(score_text) * (score_font.size // 2)

                # 2. 绘制 (x_score 是右边缘参考点)
