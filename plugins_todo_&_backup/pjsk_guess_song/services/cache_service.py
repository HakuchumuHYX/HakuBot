# pjsk_guess_song/services/cache_service.py
...
class CacheService:
    def __init__(self, resources_dir: Path, output_dir: Path, config: PluginConfig):
    ...
    async def open_image(self, relative_path: str) -> Optional[Image.Image]:
        ...
        if not source:
            logger.warning(f"请求的图片资源未找到: {relative_path}")
            return None
