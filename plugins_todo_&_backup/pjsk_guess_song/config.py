...
import nonebot_plugin_localstore as localstore  # <--- [新] 导入 localstore
...
# [新] 使用 localstore 替换旧的路径逻辑
# 插件标识符，用于 localstore
PLUGIN_NAME = "pjsk_guess_song"

# data_dir 将指向 '.../data/pjsk_guess_song/' (位于bot根目录的data文件夹下)
data_dir = localstore.get_data_dir(PLUGIN_NAME)
# localstore 默认会创建目录, 此处 mkdir 仅为确保
data_dir.mkdir(parents=True, exist_ok=True)

# 将 config.json 也存放在数据目录中 (保持原插件逻辑)
...
# --- 4. 在导入时立即加载配置 ---
# 这样 __init__.py 就可以直接 from .config import plugin_config, data_dir
